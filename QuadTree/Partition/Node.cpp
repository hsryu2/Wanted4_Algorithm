...
bool Node::Subdivide()
{
    ...
    if (!IsDivided())
    {
        ...
        bottomRight = new Node(Bounds(x + halfHeight, y + halfHeight, halfWidth, halfHeight), depth + 1);
...
std::vector<NodeIndex> Node::GetQuads(const Bounds& bounds)
{
    ...
    // 나와 다른 other인 변수 bounds와 겹치는지 확인하려는 것.
    // 영역 계산에 필요한 변수.
    int x = this->bounds.X();
    int y = this->bounds.Y();
